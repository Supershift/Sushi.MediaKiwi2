<script setup lang="ts">
  ...
  var children = store.navigationItems.filter(
    (item) => item.parentNavigationItemId == props.navigationItem.id
  )
...
<template>
  <v-list-group v-if="children.length > 0">
    <template #activator="{ props }">
      <v-list-item
        ...
        @click="
          navigationItem.screenId != null ? itemClick(navigationItem) : {}
        "
      >
      </v-list-item>
    </template>
  ...
  <v-list-item
    v-else
    :title="navigationItem.name"
    @click="navigationItem.screenId != null ? itemClick(navigationItem) : {}"
  >
  </v-list-item>
</template>
