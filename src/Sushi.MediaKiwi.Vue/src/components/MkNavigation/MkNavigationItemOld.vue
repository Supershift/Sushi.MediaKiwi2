...
<template>
  <v-list-group v-if="children.length > 0">
    <template #activator="{ props }">
      <v-list-item v-bind="props" :title="navigationItem.name" :active="isActive" @click="navigationItem.screenId != null ? itemClick(navigationItem) : {}"> </v-list-item>
    </template>
    <MkNavigationItemOld v-for="child in children" :navigation-item="child"></MkNavigationItemOld>
  </v-list-group>
  <v-list-item v-else :title="navigationItem.name" :active="isActive" @click="navigationItem.screenId != null ? itemClick(navigationItem) : {}"> </v-list-item>
</template>
