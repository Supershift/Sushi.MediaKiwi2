...
<template>
  <v-list-group v-if="children.length > 0" :value="nameLabel">
    <template #activator="{ navigationItem }" :is-active="isActive">
      <v-list-item v-bind:active="navigationItem" v-bind="navigationItem" :title="nameLabel" @click="navigationItem?.screenId != null ? onItemClick(navigationItem) : {}"></v-list-item>
    </template>
    <!-- <v-list-item v-for="child in children" v-bind="child" :key="child.id" :title="child.name" @click="child.screenId != null ? onItemClick(navigationItem) : {}"></v-list-item> -->
