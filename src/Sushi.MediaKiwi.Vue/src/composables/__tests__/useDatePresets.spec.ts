...
describe('useDatePresets', () => {
  it('should correctly calculate day presets', () => {
    const { presets } = useDatePresets({ dayPresets: [1, 7], monthPresets: [] });
    const dayPresets = presets.value.days;
    expect(dayPresets.length).toBe(2);
  });

  it('should correctly calculate month presets', () => {
    const { presets } = useDatePresets({ dayPresets: [], monthPresets: [0, 1] });
    const monthPresets = presets.value.months;
    expect(monthPresets.length).toBe(2);
  });

  it('presets should have correct structure', () => {
    const { presets } = useDatePresets({ dayPresets: [1], monthPresets: [0] });
